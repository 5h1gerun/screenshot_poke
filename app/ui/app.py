        ...
        if frozen and os.name == "nt":
            ...
            with open(bat_path, "w", encoding="utf-8") as bf:
                bf.write(
                    ...
                    "goto :wait_place\n"
                    ":placed\n"
                    "rem Try to remove Mark-of-the-Web to avoid SmartScreen/blocked streams\n"
                    "powershell -NoProfile -Command \"try { Unblock-File -LiteralPath %DST% } catch { }\" >nul 2>&1\n"
                    "rem Give AV/Indexing time to release the new file\n"
                    "timeout /t 2 /nobreak >nul\n"
                    "rem Launch and wait; on failure, roll back to previous EXE\n"
                    "start \"\" /wait %DST%\n"
                    "set EXIT=%ERRORLEVEL%\n"
                    "if not \"%EXIT%\"==\"0\" (\n"
                    "  echo Launch failed with %EXIT%, rolling back...\n"
                    "  move /y %BACK% %DST% >nul 2>&1\n"
                    "  timeout /t 1 /nobreak >nul\n"
                    "  start \"\" %DST%\n"
                    ") else (\n"
                    "  del %BACK% >nul 2>&1\n"
